...
SAASI is described in @song2025sampling.



## Installation

You can install the development version of saasi from [GitHub](https://github.com/MAGPIE-SFU/saasi) with:
...
For this example, we use data from Nextstrain [@hadfield2018nextstrain; @sagulenko2018treetime]:
...
Users can apply the function `prepare_tree_for_saasi()` to make the tree compatible with saasi. To check if the tree is compatible, use the function `check_tree_compatibility()`. Here, the function will include the tip states (i.e. their location) in the tree as `r tree$tip.state`

```{r Prepare tree}
ebola_tree <- prepare_tree_for_saasi(tree, tip_data)
```

This tree is a downsampled version of the 2013 Ebola virus tree from nextstrain. There are 193 tips from three countries.
...
2. Symmetric rate `SYM`, 3. All rates different `ARD`, and or a custom structure for the matrix.
