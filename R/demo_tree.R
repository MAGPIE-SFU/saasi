...
saasi_demo <- saasi(phy = demo_tree_prepared,            # phylogenetic tree
                     q_matrix = demo_Q,                # transition rate matrix
                     lambda = 1.5,       # speciation rate
                     mu = 0.3,               # extinction rate
                     psi = c(0.1,0.5,0.5),             # sampling rate
                     prior = NULL)    

pp <- plot_saasi(demo_tree_prepared, saasi_demo, save_file = NULL)
