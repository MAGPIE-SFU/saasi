...
#' @export
mle_lm <- function(phy, lambda, mu, psi, method = "L-BFGS-B", 
  ...
  generate_starting_points <- function(lower, upper) {
    ...
    # Conservative points
    points[[length(points) + 1]] <- list(lambda = 0.05, mu = 0.01, psi = 0.005)
    points[[length(points) + 1]] <- list(lambda = 0.03, mu = 0.005, psi = 0.003)
    points[[length(points) + 1]] <- list(lambda = 0.08, mu = 0.02, psi = 0.01)
...
extract_results <- function(fit) {
  ...
  print(results)
  cat("\nLog-likelihood:", -fit@min, "\n")
  cat("AIC:", 2 * length(coeffs) + 2 * fit@min, "\n")
  
  return(results)
}

# Diagnostic function to test parameter values
test_parameters <- function(phy, lambda, mu, psi) {
  node_depths <- ape::node.depth.edgelength(phy)
  node_times <- max(node_depths) - node_depths
  nnode <- phy[["Nnode"]] * 2 + 1
  nleaf_node <- nnode - phy[["Nnode"]]
  ...
  cat("Testing parameters: lambda =", lambda, ", mu =", mu, ", psi =", psi, "\n")
  
  # Test constraints
  cat("Constraint checks:\n")
  cat("  lambda > 0:", lambda > 0, "\n")
  cat("  mu >= 0:", mu >= 0, "\n")
  cat("  psi > 0:", psi > 0, "\n")
  cat("  lambda > mu:", lambda > mu, "\n")
  cat("  psi < lambda:", psi < lambda, "\n")
  
  # Test discriminant
  discriminant <- (lambda - mu - psi)^2 + (4 * lambda * psi)
  cat("  discriminant > 0:", discriminant > 0, "(value:", discriminant, ")\n")
  ...
  tryCatch({
    ...
    # Test q function on a small sample
    test_times <- c(0.1, 0.5, 1.0)
    q_vals <- q(lambda, mu, psi, test_times)
    cat("  q values at test times:", q_vals, "\n")
    
    # Test likelihood
    loglik <- probability_density(lambda, mu, psi, internal_node_times, leaf_node_times)
    cat("  log-likelihood:", loglik, "\n")
    
    if (is.finite(loglik)) {
      cat("SUCCESS: Parameters produce finite likelihood\n")
    } else {
      cat("PROBLEM: Parameters produce non-finite likelihood\n")
    }
    
  }, error = function(e) {
    cat("ERROR in calculations:", e$message, "\n")
  })
}